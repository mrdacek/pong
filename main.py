...
def start_hardcore_mode():
    subprocess.Popen(["python", "hardcore.py"])  # Spustí hardcore.py jako nový proces
    sys.exit()  # Ukončí běžící klasický režim


def random_direction():
    speed = 0.2  # Fixed speed
    dx = random.choice([-20, 20]) * speed
    dy = dy = random.randint(-20, 20) * speed
    return dx, dy
...
create_button("classic", "classic mode", 0, 0, 24, start_game)
create_button("hardcore", "Hardcore Mode", 0, -40, 24, start_hardcore_mode)
